        ...
        return io.Schema(
            ...
            inputs=[
                io.Combo.Input("model_name", options=_scan_models(),
                               tooltip="Any latent upscale model file in the latent_upscale_models folder (e.g. minimax_h3_latent_upscaler_3d_*.safetensors). Loading an incompatible model may error."),
                io.Int.Input("width", default=1280, min=64, max=4096, step=32,
                             tooltip="Target pixel width of the upscaled chunk (snapped to a multiple of 32, the H3 upscaler's required grid). Must match the conditioning's generation size."),
                io.Int.Input("height", default=704, min=64, max=4096, step=32,
                             tooltip="Target pixel height of the upscaled chunk (snapped to a multiple of 32, the H3 upscaler's required grid). Must match the conditioning's generation size."),
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Int.Input("width", default=1280, min=64, max=4096, step=32,
                             tooltip="Target pixel width of the upscaled chunk (snapped to a multiple of 32). Must match the conditioning's generation size."),
                io.Int.Input("height", default=704, min=64, max=4096, step=32,
                             tooltip="Target pixel height of the upscaled chunk (snapped to a multiple of 32). Must match the conditioning's generation size."),
            ],
            outputs=[
                H3_UPSCALE_PARAM.Output("latent_upscale_param",
        ...
        return io.Schema(
            ...
            inputs=[
                ...
                io.Int.Input("fade_width", default=32, min=0, max=100000, step=32,
                             tooltip="Horizontal mask fade width in PIXELS from the frozen seam toward the free interior. 32 (default) freezes the whole overlap strip."),
                io.Int.Input("fade_height", default=32, min=0, max=100000, step=32,
                             tooltip="Vertical mask fade height in PIXELS from the frozen seam toward the free interior. 32 (default) freezes the whole overlap strip."),
